...
create policy "events_select_member"
  on public.events
  for select
  using (public.is_event_member((select auth.uid()), id::text));

create policy "events_insert_panitia"
  on public.events
  for insert
  with check (public.has_event_role((select auth.uid()), id::text, 'PANITIA'));

create policy "events_update_panitia"
  on public.events
  for update
  using (public.has_event_role((select auth.uid()), id::text, 'PANITIA'))
  with check (public.has_event_role((select auth.uid()), id::text, 'PANITIA'));
...
create policy "enrollments_select_owner_or_panitia_or_observer"
  ...
  using (
    ...
    or public.has_event_role((select auth.uid()), event_id::text, 'OBSERVER')
  );
...
create policy "assessment_scores_select_recap"
  ...
  using (
    exists (
      ...
      where ai.id::text = assessment_scores.instrument_id::text
        and (
          public.has_event_role((select auth.uid()), ai.event_id::text, 'PANITIA')
          or public.has_event_role((select auth.uid()), ai.event_id::text, 'PELATIH')
          or public.has_event_role((select auth.uid()), ai.event_id::text, 'OBSERVER')
        )
    )
  );
  ...
  with check (
    exists (
      ...
      where ki.id::text = kap_responses.instrument_id::text
        and public.is_event_member((select auth.uid()), ki.event_id::text)
    )
  );
...
create policy "kap_responses_select_recap"
  ...
  using (
    exists (
      ...
      where ki.id::text = kap_responses.instrument_id::text
        and (
          public.has_event_role((select auth.uid()), ki.event_id::text, 'PANITIA')
          or public.has_event_role((select auth.uid()), ki.event_id::text, 'OBSERVER')
        )
    )
  );
...
create policy "kap_instruments_select_member"
  ...
  for select
  using (public.is_event_member((select auth.uid()), event_id::text));
  ...
  using (
    exists (
      ...
      where ai.id::text = assessment_criteria.instrument_id::text
        and public.is_event_member((select auth.uid()), ai.event_id::text)
    )
  );
...
create policy "enrollment_documents_select_member"
  ...
  using (
    exists (
      ...
      where e.id::text = enrollment_documents.enrollment_id::text
        and (
          e.user_id::text = (select auth.uid())::text
          or public.has_event_role((select auth.uid()), e.event_id::text, 'PANITIA')
          or public.has_event_role((select auth.uid()), e.event_id::text, 'OBSERVER')
        )
    )
  );
